...
sgx_enclave_id_t l_setup_enclave(struct client_info *ci, struct thread_data *data);
...
void *spawn_lua_enclave(struct client_info *ci, int socket);
...
struct thread_data {
    int argc;
    int i;
    short unsigned int daemon_port;
    char **argv;
