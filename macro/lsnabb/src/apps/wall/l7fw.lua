...
function L7Fw:report()
    ...
    local a_pct = math.ceil((self.accepted / total) * 100)
    local r_pct = math.ceil((self.rejected / total) * 100)
    local d_pct = math.ceil((self.dropped / total) * 100)
    print(("Accepted packets: %d (%d%%)"):format(self.accepted, a_pct))
    print(("Rejected packets: %d (%d%%)"):format(self.rejected, r_pct))
    print(("Dropped packets:  %d (%d%%)"):format(self.dropped, d_pct))
    pf_done = true
end
