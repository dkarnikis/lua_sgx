...
function ocall(a,b,c)
   local json = require("dkjson")
   local z = {}
   z.a=a
   z.b=b
   z.c=c
   local res = json.encode(z, {indent = true})
   f=io.open('out1', 'w')
   f:write(res)
   f:close()
   --ofload to luajit
   cmd=("luajit jffi.lua ; cat hahaxd"); --sed -i '$ d' xd1; cat xd1")
   local h = io.popen(cmd)
   local res = h:read("*a")
   h:close()
   --io.popen("rm -f xd1;rm -f out1;"):close();
   pkts = res
   --print("-->", dump(pkts))
   
   return tonumber(pkts)
end
...
local function read_buffer_word_by_type(builder, buffer, offset, size)
   if size == 1 then                                                  
    ...
    elseif size == 2 then                                             
      return ("ocall"..'("uint16_t*", '..buffer..','..offset..')')    
   elseif size == 4 then                                              
      return ("ocall"..'("uint32_t*", '..buffer..','..offset..')')    
   else                                                               
      error("bad [] size: "..size)                                    
   end                                                                
end                                                                   
