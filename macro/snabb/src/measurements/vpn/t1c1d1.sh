...
for d in */ ; do
    ...
    cd $d
        ...
        paste -d, t v f1 f4 f5 | column  -s',' -n -t > final_enc
...
for d in */ ; do
    ...
    rem=remote_sgx_dec_res
    rem_dec=remote_sgx_enc_dec_res
    rem_dec_opt=remote_sgx_opti_encrypted_dec_res
    cd $d
        f=$(awk '{ print $4  }' ${local})
        ...
        paste -d, t v f1 f4 f5 | column  -s',' -n -t > final_dec
        sed -i '1s/^/Bench Vanilla SGX_Local SGX_Rem_Enc SGX_Rem_Encr_Opti\n/' final_dec
    echo "$d"
    cd -
done
