...
local z = {}
function ocall(a,b,offset)
	--c_str = ffi.new("unsigned char [10240]", b)
    --l = ffi.cast(a, c_str + offset)[0]
    local num = 0
    if a == 2 then
        local d1,c1,b1,a1 = b:byte(offset+1, offset+4)
        num = bit.lshift(a1, 24) + bit.lshift(b1, 16) + bit.lshift(c1, 8) + d1
    else 
        local b1,a1 = b:byte(offset+1, offset+2)
        num = bit.lshift(a1, 8) + b1
    end
	d1 = nil
	c1 = nil
	b1 = nil
...
local function filter_builder(...)
	...
	local function r1ead_buffer_word_by_type(builder, buffer, offset, size)
		print(type(buffer))
		if size == 1 then                                                  
			return buffer..'['..offset..']'                                 
		elseif size == 2 then 
			t = 'ocall'..'(' .."uint16_t*"..', '..buffer..','..offset..')'
			return t    
		elseif size == 4 then                                              
			t = 'ocall'..'(' .."uint32_t*"..', '..buffer..','..offset..')'
			return t
		else                                                               
			error("bad [] size: "..size)                                    
		end                                                                
	...
	local function read_buffer_word_by_type(builder, buffer, offset, size)
		if size == 1 then
			return buffer..'['..offset..']'
		elseif size == 2 then
			return  "ocall(" .. "1".. "," ..buffer ..",".. offset..")"
			--return (builder.c('ffi.cast')..'("uint16_t*", '..buffer..'+'..offset..')[0]')
		elseif size == 4 then
			--x = (builder.c('ffi.cast')..'("uint32_t*", '..buffer..'+'..offset..')[0]')
			return "ocall(" .. "2" .."," ..buffer ..",".. offset..")"
		else
			error("bad [] size: "..size)
		end
	end
	...
	function backend.emit_and_load_match(ssa, name, ...)
